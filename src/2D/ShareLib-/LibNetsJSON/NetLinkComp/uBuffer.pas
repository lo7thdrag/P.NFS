...
begin
  ...
  begin
    if Assigned(FLogStat) then begin
      FLogStat(TimeToString + ' : Data ' + FRegProcs[Header.PacketID].recName);
    end;
  ...
  begin
    if Assigned(FLogStat) then
      FLogStat(TimeToString + ' : ' + 'UnRegistered ID ' + IntToStr(Header.PacketID));
  end;
end;
