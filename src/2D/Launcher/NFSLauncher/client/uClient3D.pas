...
type
  ...
  private
    ...
    aCheckShipID, aCheckLauncherID: Integer;

    { Check Connectiion }
    FisCheckConnect, FisCheckRespone: Boolean;
    ...
    procedure OnNewLine(Sender: TObject; NewLine: string;
...
procedure ExecuteCommandWithWorkingDirectory(const Command: string;
  const WorkingDirectory: string; isWait: Boolean);
var
  StartupInfo: TStartupInfo;
  ProcessInfo: TProcessInformation;
  CmdLine: string;
begin
  // Initialize StartupInfo and ProcessInfo structures
  ...
  // If you are executing an external executable directly, CmdLine should be the path to the executable.
  // If you are running a command that requires a shell (like 'dir'), you'll need to invoke cmd.exe.
  // For example: CmdLine := 'cmd.exe /C ' + Command;
  CmdLine := Command; // Or 'cmd.exe /C ' + Command for shell commands

  // Create the process
  if not CreateProcess(
    nil,                               // Application Name (nil if part of CmdLine)
    PChar(CmdLine),                    // Command line
    nil,                               // Process Security Attributes
    nil,                               // Thread Security Attributes
    False,                             // Inherit Handles
    CREATE_NO_WINDOW,                  // Creation Flags (e.g., CREATE_NO_WINDOW for hidden window)
    nil,                               // Environment
    PChar(WorkingDirectory),           // Current Directory
    StartupInfo,                       // Startup Info
    ProcessInfo                        // Process Info
  ) then
  begin
    RaiseLastOSError; // Raise an exception if CreateProcess fails
  end;
...
procedure SendDataClientManagementFromClient(theClient: TTCPClient;
  mOrderID: byte; mNumValue: Integer; mStrValue: string);
var
  thePacket: TRecData2DOrder;
begin
  with thePacket do
...
begin
  ...
  begin
    ...
    if bDebug then
      ShowMessage('Start Deleting Old XML...');

    if FileExists(fShipXMLName) then
    begin
      DeleteFile(fShipXMLName);
      if bDebug then
        AddToLogs('Old ' + fShipXMLName + ' file deleted!');
    end
    else if bDebug then
      AddToLogs('Cannot find old ' + fShipXMLName + '. Proceed!');

    if FileExists(fOceanXMLName) then
    begin
      DeleteFile(fOceanXMLName);
      if bDebug then
        AddToLogs('Old ' + fOceanXMLName + ' file deleted!');
    end
    else if bDebug then
      AddToLogs('Cannot find old ' + fOceanXMLName + '. Proceed!');

    if FileExists(fDetailXMLName) then
    begin
      DeleteFile(fDetailXMLName);
      if bDebug then
        AddToLogs('Old ' + fDetailXMLName + ' file deleted!');
    end
    else if bDebug then
      AddToLogs('Cannot find old ' + fDetailXMLName + '. Proceed!');

    if bDebug then
      ShowMessage('End Deleting Old XML...');
    // ++ BAWE-20110617: XML_CLEANUP
    // bIsCleaningUp := false;
  end;
  // -- BAWE-20110617: XML_CLEANUP
...
begin
  ...
  if bDebug then
    ShowMessage('Start Generate Needed XML Scenario');
  ...
  if bDebug then
    ShowMessage('End Generate Needed XML Scenario');
...
begin
  ...
  if thisComputer.c_app_tipe = APP_3D_Client then
  begin
    SendDataClientManagementFromClient(theClient, _CM_CLIENT_MANAGE,
      __CM_CLIENT_REQ3D_ADDR, '');
...
begin
  ...
  mDBServer := sDB;
  if DataModule1.InitZDB(mDBServer, mDBProto, mDBName, mDBUser, mDBPass, mDBPort)
  then
  begin
...
var
  ...
  path: string;
  cShipName: string;
  cShipClassID: Integer;
  cShipClassName: string;

  cShipID, weaponID, LauncherID, FirstHeading: Integer;

  isAssign, isLauncherExist: Boolean;
begin
  path:= '.';
...
//        Sleep(6000);
//
//        GetShipIDFromSceIDAndConsoleID(sceneID, thisComputer.c_id_console,
//          cShipID, isAssign);
//
//        if isAssign then
//        begin
...
//            IntToStr(sceneID) + ' --vehicle ' + IntToStr(cShipID);
//          AddToLogs(sLine);
...
//        end;
        ...
        begin
          ...
          aCheckShipID := cShipID;
          aCheckLauncherID := 0;
          sLine := path + '\' + thisComputer.c_app_name;
          AddToLogs(sLine);
          AddToLogs('END OBSERVER MODE');
        end
        else
...
//
//        GetShip_LauncherIDFromSceIDAndConsoleID(sceneID,
//          thisComputer.c_id_console, cShipID, LauncherID, isAssign);
//
//        if isAssign then
//        begin
//          { nando }
...
//          AddToLogs('SHIP ID : ' + IntToStr(cShipID) + ' WEAPONID : ' +
//            IntToStr(weaponID) + ' LAUNCHERID : ' + IntToStr(LauncherID));
//
//          CheckLauncherExistAndHeadingFromShipWeapon(cShipID, weaponID,
...
//              + ' --vehicle ' + IntToStr(cShipID) + ' --weapon  ' +
...
//              IntToStr(cShipID) + ' -' + IntToStr(sceneID) + ' -' +
//              (IntToStr(LauncherID));
//
//            aCheckShipID := cShipID;
...
//    cmdClientApp.Execute;
    ...
    AddToLogs('Received Launch Command, but something wrong !.()'+path+'(' + sLine + ')');
...
//    cmdClientApp.CommandLine := sLine_2;
    ...
    AddToLogs('run' + sLine_2);
//    cmdClientApp.Execute;
    ExecuteCommandWithWorkingDirectory(sLine_2, path, False);
  end
  else
    AddToLogs('Received Launch SECOND COMMAND, but something wrong !.()'+path+'(' +
      sLine_2 + ')');
end;
...
var
  ...
  isFound, isLauncherExist: Boolean;
...
//        sName := thisComputer.c_app_params + thisComputer.c_app_name;
  ...
  if cbDebug.Checked then
    AddToLogs('State Test 5  :: params :: "' + sLine + '"');
...
var
  ...
  strValue, strValue2, strValue3: string;
...
begin
  ...
  if cbDebug.Checked then
    AddToLogs('-- DEBUG ORDER ID : "' + IntToStr(orderID) + '" :: "' +
      IntToStr(numValue) + '" :: "' + strValue + '"');

  case orderID of
    _CM_CLIENT_MANAGE:
      begin
        case numValue of
          __CM_CLIENT_SETDB_ADDR:
            begin
              SetDBAddress(strValue);
            end;
          __CM_CLIENT_SET3D_ADDR:
            begin
          ...
          __CM_CLIENT_LAUNCH:
            begin
              ...
              begin
                ...
                AddToLogs(' ::: Receive Launch Command. but, wrong "' +
                  strValue + '"');
            end;
          __CM_CLIENT_STOP:
            begin
...
begin
  ...
  if not DirectoryExists(DATA_PATH) then
    ShowMessage('Data Path does not exist');
  ...
  InitDefault_GameServerConfig(path, m2DServerIP, m2DServerPort, m3DServerIP,
    m3DServerPort);
  InitDefault_DBConfig(path, mDBServer, mDBProto, mDBName, mDBUser,
    mDBPass, mDBPort);
  ...
  theClient.OnConnected := OnConnected;
  theClient.OnDisconnected := OnDisconnected;
  theClient.RegisterProcedure(REC_2D_ORDER, onRec2DOrderAvailable,
  ...
  nameFile := ExtractFilePath(Application.ExeName) + 'NFSAgent.ini';
...
procedure TfrmMain.TmrCheckConnectTimer(Sender: TObject);
begin
  if (theClient.State <> wsConnected) then
  begin
