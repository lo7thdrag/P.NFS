...
//==============================================================================
  ...
  function GetTimeInSecFromFormattedMMSS(mmss: string): Integer;
  ...
  function ValidateDegreeStr(const aDegree: string): string;
  ...
  procedure CopyStringWithoutChar(const Source: string; var Dest: string; ExcludeChar: Char);
  {
    desc : menghilangkan specified character ketika copy string ke another string
    input : source string yang mau dicopy, ExcludeChar yang mau dihilangkan
    output : string baru tanpa specified character
  }

  function FormatCourse(const d: double): string; overload;

  procedure CenterOfTwoCoordinate(const x1, y1, x2, y2: double; var cenx, ceny : double);
  {
    desc: menghitung titik tengah (midpoint) dari 2 coordinat
    input: x1 y1 adalah koordinat pertama dan x2 y2 adalah koordinat kedua
    output: cenx dan ceny adalah hasil perhitungan hasil perhitungan titik tengah 2 koordinat
  }
...
begin
  TimeParts := mmss.Split([':']);

  if Length(TimeParts) <> 2 then
    Result := -1;

  if not TryStrToInt(TimeParts[0], Minutes) then
    Result := -1;

  if not TryStrToInt(TimeParts[1], Seconds) then
    Result := -1;
...
//begin
//  { input long lat degree, output nautical mile}
//  z1Nm := (z1 * C_Feet_To_Meter) * C_Meter_To_NauticalMile;
//  z2Nm := (z2 * C_Feet_To_Meter) * C_Meter_To_NauticalMile;
//
//  dx := (x2 - x1) ;
//  dy := (y2 - y1) ;
//  dz := (z2Nm - z1Nm) ;
...
begin
  ...
  aDegDbl := StrToFloatDef(aDegree, 0.0);
  aDegDbl := ValidateDegree(aDegDbl);

  Result := FormatFloat('000', aDegDbl);
...
begin
  ...
  begin
    // Check if the character is not the excluded character
    if Source[i] <> ExcludeChar then
    begin
