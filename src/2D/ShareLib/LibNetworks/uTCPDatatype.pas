...
type
  ...
  TRecDataTorperdo = packed record
    Pc: TPacketCheck;
  ...
  TTorpData = record // Torpedo Parameter Data
    ...
    ISC, ISR, ACE, CEI, FLO, ISD: Integer;
  end;
  ...
  // REC_3D_POSITION = 1; -------------------------------------------------------
  TRecData3DPosition = packed record
    Pc: TPacketCheck;
    ShipID: word;
    X, Y, Z: double;
    Heading: single;
  ...
  TRecTorpedoMK44Order = packed record
    Pc: TPacketCheck;
    ShipID: word;
    OrderID: Byte;
    mID: word;
  ...
  TRecSetTorpedoSUT = packed record
    Pc: TPacketCheck;
  ...
  TRecOrderAssignment = packed record
    Pc: TPacketCheck;
    ShipID: string[15];
    OrderID: Byte;
  ...
  TRecSetTrackNumber = packed record
    Pc: TPacketCheck;
    ShipID: string[15];
    OrderID: Byte;
    ShipTID: Byte;
  ...
  TRecFireControlOrder = packed record
    Pc: TPacketCheck;
    ShipID: string[15];
    OrderID: Byte;
    FC_number: Byte;
  ...
  TRecHarpoonPanelSetting = packed record // 30;
    Pc: TPacketCheck;
    ShipID: string[15];
    OrderID: Byte;
    OrderData: Byte;
  ...
  spActorsController = packed record
    Pc: TPacketCheck;
    ShipID: word;
    TypeID: word;
  ...
  TRecSplashCannon = packed record

    Pc: TPacketCheck;
    ShipID, WeaponID, launcherID: word;
  ...
  // for Console
  TRecStatus_Console = packed record
    Pc: TPacketCheck;
  ...
  // for C-802 Console
  TRecStatus_Console_C802 = packed record
    Pc: TPacketCheck;
  ...
  TRecStatus_Console_Yakhont = packed record
    Pc: TPacketCheck;
  ...
  TRecRBU_SonarMode = packed record
    Pc: TPacketCheck;
  ...
  TRecData_Yakhont = packed record
    Pc: TPacketCheck;
    ...
    mMissile1, mMissile2, mMissile3, mMissile4: Byte;
  ...
  TRecStatusGame = packed record // =84
    Pc: TPacketCheck;
  ...
  TRecEnableWeapon = packed record
    Pc: TPacketCheck;
  ...
  TRecAsrocMissileType = packed record
    Pc: TPacketCheck;
  ...
  // Exocet 27
  TRec3DSetExocet_40 = packed record
    Pc: TPacketCheck;
  ...
  // Mistral
  TRec3DSetMistral = packed record
    Pc: TPacketCheck;
  ...
  // Strella\
  TRec3DSetStrella = packed record
    Pc: TPacketCheck;
  ...
  TRecObjectAssigned = packed record { For C802 & Yakhont Only }
    Pc: TPacketCheck;
  ...
  TRecWeaponShowRange = packed record
    Pc: TPacketCheck;
  ...
  // rec Trajectory view
  TRec_Trajectory_View = packed record
    Pc: TPacketCheck;
    WeaponLabel: Byte;
    status: Byte;
  end;
  ...
  // rec ViewRangeWeapon
  TRec_View_Range_Weapon = packed record
    Pc: TPacketCheck;
    aID: string[20];
  ...
  PRecCmdSetCameraTarget = ^TRecCmdSetCameraTarget;
  TRecCmdSetCameraTarget = packed record
    Pc: TPacketCheck;
    ShipID: Word;
  end;

  // REC_CMD_DESIG_A244
  TRecDesigA244 = packed record
    Pc: TPacketCheck;
    ShipID: Word;
    mWeaponID: Word;
    mTargetID: Word;
...
//  ORD_ENVI_fogIntensity = 8;
  ...
  __STAT_RBU_ALIRAN_KAPAL = 305;
  // u/ RBU mulai 305 kebwah defaultnya di bikin true ya mas yg mengerjakan Instruktur!!!
  ...
  // ERROR FOR CANNON35
  __STAT_CANNON35_ENABLE = 1801;
  __STAT_CANNON35_CAP = 1802;
  __PARAM_CANNON35_ON = 1;
  __PARAM_CANNON35_OFF = 2;
  __STAT_CANNON35_MAS = 1803;
  __PARAM_CANNON35_MAS_ON = 1;
  __PARAM_CANNON35_MAS_OFF = 2;
  __STAT_CANNON35_MISFIRE = 1804;
  __PARAM_CANNON35_SIMUL_MISFIRE = 1;
  __PARAM_CANNON35_CLEAR_MISFIRE = 2;
  ...
  // MOC3 and MOC4 TDS
  __STAT_TDS_PORT = 1901;
  __STAT_TDS_STBD = 1902;

  // ERROR FOR MOC PKR
  __STAT_MOC_PKR_RadarARPA = 1601;
  __STAT_MOC_PKR_SMART_S = 1602;
  __STAT_MOC_PKR_STING_EO = 1603;
  __STAT_MOC_PKR_Kingklip = 1604;
  __STAT_MOC_PKR_FIS_Bridge = 1605;
  __STAT_MOC_PKR_FIS_OPS = 1606;
  __STAT_MOC_PKR_Check_Fire = 1607;
  __PARAM_MOC_PKR_ON = 1;
  __PARAM_MOC_PKR_OFF = 2;

  // ERROR FOR VLMICA
  __STAT_VLMICA_ENABLE = 1701;
  __STAT_VLMICA_SEQ = 1702;
  __STAT_VLMICA_FA = 1703;
  __STAT_VLMICA_AMMOTEST = 1704;
  __STAT_VLMICA_CAP = 1705;
  __PARAM_VLMICA_ON = 1;
  __PARAM_VLMICA_OFF = 2;

  // ERROR FOR CANNON AK230
  __STAT_CANNONAK230_ENABLE = 1801;
  __PARAM_CANNONAK230_ON = 1;
  __PARAM_CANNONAK230_OFF = 2;

  // ERROR FOR CANNON TYPE 730
  __STAT_CANNONTYPE730_ENABLE = 1901;
  __PARAM_CANNONTYPE730_ON = 1;
  __PARAM_CANNONTYPE730_OFF = 2;

    // ERROR FOR Blackshark
  __STAT_BLACKSHARK_ENABLE = 2001;
  __STAT_BLACKSHARK_TBI_FIRE_AUTHORIZE = 2002;
  ...
  C_REC_PACKETNAME: array [1 .. 120] of string = ('REC_3D_POSITION',
    ...
    'REC_3D_TORPEDO_SUT', 'C_REC_ORDER',
    { 11 }
    'C_REC_ORDER_XY', 'C_REC_TRACK_ORDER', 'C_REC_ORDER_ASSIGNMENT',
    'C_REC_SET_TRACKNUM', 'C_REC_FIRE_CONTROL',

    'C_REC_GUN_CONTROL', 'C_REC_XXX_ORDER', 'C_REC_LINK_ORDER', 'REC_SET_RBU',
    'REC_802',
    ...
    { 51 }
    'REC_3D_SETCONTROL ', 'REC_TEXTURE_CAMERA', 'REC_3D_UTIL_TOOLS',
    'REC_2D_ORDER', 'REC_STAT_ORDER_CONSOLE',

    'REC_ERROR_SPS115', 'REC_SPSS_ORDER', 'REC_SET_TIU_Port',
    'REC_SET_TIU_Stbd', '',
    { 61 }
    '', 'REC_ASRL_PANEL_SETTING', 'REC_3D_WCC', 'REC_STAT_CANNON_SPLASH',
    'REC_RBU_SONARMODE',
    ...
    { 76 }
    'REC_STATUS_C802', 'REC_GUIDANCE', 'REC_STATUS_YAKHONT', 'REC_EVENT_LOG',
    'REC_GAME_NAME',
    { 81 }
    'REC_COMMAND_YAKHONTPLP', 'REC_RECV_TORP_STATE', 'REC_DATA_CONTROL',
    'REC_STATUS_GAME', 'REC_ENABLE_WEAPON', 'REC_STATUS_LOADING',
    'REC_STATUS_MESSAGE', 'REC_REPLAY', 'REC_ASROC_TYPE_MISSILE',
    'REC_TRAJECTORY_VIEW',
    { 91 }
  ...
  C_ORDER_STR: array [1 .. C_Max_Order_ID] of string = ('select_radar_type',
    'start_ICM', 'update_ICM', 'end_ICM', 'init_datum', 'FOC_plus', 'FOC_minus',
    'change_ident', 'assign_tor', 'deassign_tor', 'assign_asrl',
    'deassign_asrl', 'WIPE', 'init_ram', 'assign_ram', '', '', 'update_SSM',
    'assign_SSM', 'deassign_SSM', 'init AUTO', 'assign_ram_udara', 'init DR',
    'init_esm_fix', 'Synch TB', 'Update Track Pos', 'assign_FC_WCC',
    'deassign_FC_WCC', 'fire_gun1', 'fire_gun2', 'fire_gun3', 'assign_gun',
    'deassign_gun', 'assign_FC_OCC', 'deassign_FC_OCC', '', '', '', '', '', '',
    '', '', '', '', '', '', '', '', '', 'Asrock Assign', 'Asrock Deassign',
    'Asrock Fire',

    // Tocos
    'fire_torpedoMK46', 'fire_torpedoSUT', 'torpedoSUT_handle',
    'assign_torpSUT', 'deassign_torpSUT', 'ORD_3D_TORPEDO_SUT_UPDATE_BEARING',
...
type
  ...
  OrdType_datum_number = (datum_number1, datum_number2, datum_number3,
    datum_number4, datum_number5);
  OrdType_torpedo = (MK44, A244);
  OrdType_wipe = (kiri_1, kiri_2, kanan_1, kanan_2);

  TOrdType_FC_number = (FC1, FC2, FC3, FC4, FC5);

  TOrdType_FC_command = (STO, SBS, ATO, SBA, RATO, RSTO, break_track, open_fire,
    hold_fire, GenFix, BlindBomb, IndirectBomb, InitTrackSurf, AssignTrackSs);

  TOrdType_OWA_command = (InitAir, InitSurf, InitSubRT, InitSubNRT, AssLocRT,
    AssLocNRT, AssRemote, ReposTrack, CorrectTrack);
