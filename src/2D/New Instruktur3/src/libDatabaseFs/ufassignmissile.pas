...
uses uBaseFunction, uClassDatabase, uDataModule;
    ...
    begin
      ...
      begin
        Caption := DataModule1.GetNameWeaponByID(WeaponID) + '-' +
          IntToStr(ShipID) + '-' + IntToStr(LauncherID);
...
begin
  ...
  try
    ...
    begin
      if Assigned(listModel.Items[i]) then
      begin
    ...
    begin
      if Assigned(listDOF.Items[i]) then
      begin
...
begin
  ...
  if not TryStrToInt(edtPosHeading.Text, PosH) then
    isValid := False;
  if not TryStrToInt(edtPosPitch.Text, PosP) then
    isValid := False;
  if not TryStrToInt(edtMissileID.Text, MissileID) then
    isValid := False;
...
function TfrmAssignMisille.CheckAvailableMissileID(MissileID: Integer): Boolean;
var
  ListMissile: TList;
  mMissile: TMissile;
...
begin
  ...
  try
    ...
    for i := 0 to ListMissile.Count - 1 do
    begin
      if Assigned(ListMissile.Items[i]) then begin
        mMissile := TMissile(ListMissile.Items[i]);
