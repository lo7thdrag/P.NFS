...
uses
  ...
  System.ImageList, Vcl.ImgList,Vcl.OleCtrls, MapXLib_TLB, uBaseFunctionSutBlackshark, uObjectVisual,
  uCoordConverter, uMapXUnitConverter, system.math, TFlatCheckBoxUnit, uSutBlacksharkManager, uBridgeSet,
...
type
  ...
  TfrmMainSutBlackshark = class(TForm)
...
var
  frmMainSutBlackshark: TfrmMainSutBlackshark;

implementation

{$R *.dfm}

uses uLibConst, uBaseConst, uScriptSutBlackshark, uDataModule, ulibSettings, uVehicleManager, uVehicle, uBaseFunction, uTCPDatatype;
const
  CMin_Z = 0;
  CMax_Z = 14;
...
procedure TfrmMainSutBlackshark.acbxTargetParaClick(Sender: TObject);
begin
...
procedure TfrmMainSutBlackshark.btnMapDecrementClick(Sender: TObject);
begin
...
procedure TfrmMainSutBlackshark.btnMapIncrementClick(Sender: TObject);
begin
...
procedure TfrmMainSutBlackshark.DrawAll(aCnv: TCanvas; aCvt: TCoordConverter;
  aFlag: Byte);
var
  ...
  z := ZoomIndexToScale(i);
    ...
    begin
      if Assigned(SutBlacksharkManager.xShip) then
        AreaBlindZone1.HeadingDeg    := SutBlacksharkManager.xShip.Heading;
    end;
    ...
    begin
      if Assigned(SutBlacksharkManager.xShip) then
        AreaBlindZone.HeadingDeg    := SutBlacksharkManager.xShip.Heading;
    end;
    ...
    begin
      if Assigned(SutBlacksharkManager.xShip) then
        AreaPenembakan.HeadingDeg    := SutBlacksharkManager.xShip.Heading;
    end;
    ...
    begin
      if Assigned(SutBlacksharkManager.xShip) then
        AreaTracker.HeadingDeg    := SutBlacksharkManager.xShip.Heading;
    end;
...
procedure TfrmMainSutBlackshark.DrawAngle(aCnv: TCanvas);
var
...
procedure TfrmMainSutBlackshark.DrawCompas(aCnv: TCanvas);
var
  R: TRect;
...
procedure TfrmMainSutBlackshark.DrawLine(Canvas: TCanvas; X1, Y1, X2, Y2: Integer;
...
procedure TfrmMainSutBlackshark.fireClick;
var
  RecSend : TRec3DSetWCC;
  aLow, aHigh: Double;
  range,rangem, bearing : Double;
begin
  if Assigned(SutBlacksharkManager.SelectedVehicle) then
  begin
    ...
    RecSend.mUpDown             := 0;
    RecSend.mTargetID           := UniqueID_To_dbID(SutBlacksharkManager.SelectedVehicle.UniqueID);
    ...
    SutBlacksharkManager.NetSendTo3D_OrderCannon(RecSend);
...
procedure TfrmMainSutBlackshark.FMapDrawUserLayer(ASender: TObject;
...
procedure TfrmMainSutBlackshark.FMapMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
...
procedure TfrmMainSutBlackshark.FMapMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
  ShowInfoCursor(X,y);
end;

procedure TfrmMainSutBlackshark.DAttackAssignTarget;
var
  isValid : Boolean;
  RecSend : TRec3DSetWCC;
...
begin
    ...
    range := CalcRange(SutBlacksharkManager.xShip.PositionX, SutBlacksharkManager.xShip.PositionY, StrToFloat(edtLonDirectAttack.text), StrToFloat(edtLatDirectAttack.text));
    rangem := range * C_NauticalMile_To_Metre;
    bearing := CalcBearing(SutBlacksharkManager.xShip.PositionX, SutBlacksharkManager.xShip.PositionY, StrToFloat(edtLonDirectAttack.text), StrToFloat(edtLatDirectAttack.text));
    bearing := bearing - SutBlacksharkManager.xShip.Heading;
    if bearing < 0 then
    bearing := bearing + 360;
    FBearingVal := bearing;
    // range = 3000 m, target lebih rendah 25 m
...
procedure TfrmMainSutBlackshark.FMapMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
  isValid : Boolean;
  RecSend : TRec3DSetWCC;
...
begin
  ...
  begin
    ...
    FBearingVal := bearing;
    // range = 3000 m, target lebih rendah 25 m
    ComputeBallisticAngleVacuum(rangem, SutBlacksharkManager.SelectedVehicle.PosZ, 350, aLow, aHigh);
...
//      FTargetAngleElevasi:= StrToFloatDef(edtElevasi.Text, 0);
    ...
    //IND Data
    edtIndDataBatchNo.Text := UniqueID_To_dbID(SutBlacksharkManager.SelectedVehicle.UniqueID).ToString();
    case SutBlacksharkManager.SelectedVehicle.Domain of
      1://surface
      begin
    ...
    edtIndDataCourse.Text := SutBlacksharkManager.SelectedVehicle.HeadingDeg.ToString();
    edtIndDataSpeed.Text := SutBlacksharkManager.SelectedVehicle.Speed_mps.ToString();
...
procedure TfrmMainSutBlackshark.FormCreate(Sender: TObject);
var
  n : Integer;
...
begin
  ...
  SimCenter.FMap := FMap;
  SutBlacksharkManager.OnPtkCommand := ptkCommand;
...
//    end;
  ...
  config.Video := SutBlacksharkManager.ShipID.ToString() + '_' + SutBlacksharkManager.AssignedWeapon.IDWeapon.ToString();
  ...
  DataModule1.GetScenarioDefByID(SutBlacksharkManager.CurrentScenID, envSce);
...
procedure TfrmMainSutBlackshark.FormDestroy(Sender: TObject);
var
  i : Integer;
begin
...
//  FRangeRing.Free;
  VehicleMgr.Free;
  SutBlacksharkManager.FinalizeSimulation;
...
procedure TfrmMainSutBlackshark.FormPaint(Sender: TObject);
begin
  DrawAngle(imgBackgrounSituationZone.Canvas);
  DrawCompas(imgCompas.Canvas);
end;

procedure TfrmMainSutBlackshark.FormShow(Sender: TObject);
//var
begin
...
procedure TfrmMainSutBlackshark.HandleKeyByBtnName(const BtnName: string);
var
  Token: string;
  C: Char;
  RecSend : TRec3DSetWCC;
...
procedure TfrmMainSutBlackshark.imgCompasClick(Sender: TObject);
begin
  if Assigned(SutBlacksharkManager) then
  begin
    SutBlacksharkManager.IsTrueMotion := not SutBlacksharkManager.IsTrueMotion;
  end;
end;

procedure TfrmMainSutBlackshark.InitializeForm;
//var
//envSce : TScenario;
begin
...
procedure TfrmMainSutBlackshark.LoadGeoset(const aGst: string);
var i: integer;
  z: OleVariant;
...
function TfrmMainSutBlackshark.MeterHeight: Integer;
begin
  Result := FMap.Height;
end;

function TfrmMainSutBlackshark.MeterWidth: Integer;
begin
  Result := FMap.width;
end;

procedure TfrmMainSutBlackshark.onRbNavSetting(Sender: TObject);
begin
  if rbNavAuto.Checked then
  begin
    edtNavSettingHeading.Text := FormatFloat('0.00', SutBlacksharkManager.xShip.Heading);
    edtNavSettingPitch.Text := FormatFloat('0.00', SutBlacksharkManager.xShip.Pitch);
    edtNavSettingLON.Text := FormatFloat('0.000000', SutBlacksharkManager.xShip.PositionX);
    edtNavSettingALT.Text := FormatFloat('0.00', SutBlacksharkManager.xShip.PositionZ);
    edtNavSettingSpeed.Text := FormatFloat('0.00', SutBlacksharkManager.xShip.Speed);
    edtNavSettingRoll.Text := FormatFloat('0.00', SutBlacksharkManager.xShip.Roll);
    edtNavSettingLAT.Text := FormatFloat('0.000000', SutBlacksharkManager.xShip.PositionY);
...
procedure TfrmMainSutBlackshark.onRbWeatherSetting(Sender: TObject);
var
envSce : TScenario;
begin
...
procedure TfrmMainSutBlackshark.DAttackFire;
var
  RecSend : TRec3DSetWCC;
  aLow, aHigh: Double;
  range,rangem, bearing : Double;
...
//    end;
    ...
    RecSend.mAutoCorrectElev    := range;
    RecSend.mAutoCorrectBearing := bearing;
...
//    IsFiring := True;
    ...
    case vSutBlacksharkSetting.ConsoleMode of
      ...
      begin
        ...
        begin
          WantedBullet := Firetime * 35;
          if WantedBullet > LeftDrum then
          begin
            WantedBullet := LeftDrum;
          end;

          FinalTime := WantedBullet / 35;
          Application.ProcessMessages;
          Sleep(Trunc(FinalTime * 1000));
          TargetRoundInDrum := LeftDrum - Trunc(WantedBullet);
        end
        else if DrumPosState then // kurangi right drum
        begin
          WantedBullet := Firetime * 35;
          if WantedBullet > RightDrum then
          begin
            WantedBullet := RightDrum;
          end;

          FinalTime := WantedBullet / 35;
          Application.ProcessMessages;
          Sleep(Trunc(FinalTime * 1000));
          TargetRoundInDrum := RightDrum - Trunc(WantedBullet);
        end;
      ...
      begin
        ...
        if not DrumPosState then // kurangi left drum
        begin
          WantedBullet := Firetime * 4;
          if WantedBullet > LeftDrum then
          begin
            WantedBullet := LeftDrum;
          end;

          FinalTime := WantedBullet / 4;
          Application.ProcessMessages;
          Sleep(Trunc(FinalTime * 1000));
...
procedure TfrmMainSutBlackshark.pnlFireFcc2Click(Sender: TObject);
var
  RecSend : TRec3DSetWCC;
  aLow, aHigh: Double;
  range,rangem, bearing : Double;
...
procedure TfrmMainSutBlackshark.pnlMapRangeClick(Sender: TObject);
begin
  ResetColorRange;
...
procedure TfrmMainSutBlackshark.ptkCommand(const str: string);
begin
  HandleKeyByBtnName(str);
end;

procedure TfrmMainSutBlackshark.ResetColorRange;
begin
...
function TfrmMainSutBlackshark.Rotate(Width, Height, Radius: Integer;
...
procedure TfrmMainSutBlackshark.setRegionCircle;
const brd = 30;
var eRgn: HRGN;
    r : TRect;
...
procedure TfrmMainSutBlackshark.ShowInfoCursor(const x, y: integer);
var
  mx, my: double;
  sx, sy: single;
  z: double;
...
procedure TfrmMainSutBlackshark.tmrUpdateFormTimer(Sender: TObject);
begin
...
//    FNorthAngle := 0;
  ...
  begin
    ...
    begin
      if Assigned(SutBlacksharkManager.xShip) then
      begin
        edtNavDataLAT.Text := FormatFloat('0.000000', SutBlacksharkManager.xShip.PositionY);
        edtNavDataLON.Text := FormatFloat('0.000000', SutBlacksharkManager.xShip.PositionX);
        edtNavDataSpeed.Text := FormatFloat('0.00', SutBlacksharkManager.xShip.Speed);
        edtNavDataHeading.Text := FormatFloat('0.00', SutBlacksharkManager.xShip.Heading);
        edtNavDataPitch.Text := FormatFloat('0.00', SutBlacksharkManager.xShip.Pitch);
        edtNavDataRoll.Text := FormatFloat('0.00', SutBlacksharkManager.xShip.Roll);
...
procedure TfrmMainSutBlackshark.tmrUpdateHeadingTimer(Sender: TObject);
var
  i, tempPR : Integer;
