...
const
  c_image = 'image';
  c_SutBlacksharkSetting = 'SutBlacksharkSetting';
type
  ...
  TRecSutBlacksharkSetting = record
    ConsoleMode : Integer;
  end;
  ...
  function LoadFF_SutBlacksharkSetting(const fName: string; var SutBlacksharkSet: TRecSutBlacksharkSetting): boolean;
  procedure Save_SutBlacksharkSetting(const fName: string; var SutBlacksharkSet: TRecSutBlacksharkSetting);

const
  defaultServer       = '192.168.0.231';
  default3DServer     = '192.168.0.231';
  ...
  defaultDBServer   = '192.168.0.231';
  defaultDBProto    = 'mysql';
  defaultDBName     = 'dbNSuFs';
  defaultDBUser     = 'root';
  defaultDBPassword = 'T3server!';
  defaultDBPort     = '3306';
...
var
  ...
  vSutBlacksharkSetting : TRecSutBlacksharkSetting;
...
implementation
  ...
  //  result := ChangeFileExt(ParamStr(0), '.ini');
    result := ExtractFilePath(ParamStr(0)) + 'SutBlacksharkSetting.ini';
  end;
  //==============================================================================
  ...
  begin
    ...
    with SutBlacksharkSet do
    begin
      ConsoleMode := StrToInt(ForceReadString(iniF, c_SutBlacksharkSetting, 'consoleMode', '1'));
    end;
  ...
  begin
    ini := TIniFile.Create(vSettingFile);
    with ini do begin
      WriteString(c_SutBlacksharkSetting, 'consoleMode', '1');
    end;
    ini.Free;
  end;
